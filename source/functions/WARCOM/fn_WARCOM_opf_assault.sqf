...
// Attack waves main
[] spawn {
          waitUntil {sleep 300; WARCOM_opf_attack_wave_avalaible};

          while {true} do {
          sleep 2;

          // Clean up dead groups
          {if (!isNull _x && {{alive _x} count units _x == 0}) then {deleteVehicle _x}} forEach allGroups;
          ...
          _failSafe = 0;
            while {!_found} do {
            if (count WARCOM_zones_controled_by_OPFOR == 0) exitWith {_found = true; _randomZone = WARCOM_hq_pos;};
            _randomZone = WARCOM_zones_controled_by_OPFOR call BIS_fnc_selectRandom;
            if (count _randomZone == 0) exitWith {_found = true; _randomZone = WARCOM_hq_pos;};
